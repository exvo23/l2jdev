...
public final class ArmorSet
{
	...
	private final int _enchant22Skill;
	
	public ArmorSet(int chest, int legs, int head, int gloves, int feet, int skillId, int shield, int shieldSkillId, int enchant6skill, int enchant22skill)
	{
		_chest = chest;
		_legs = legs;
		_head = head;
		...
		_enchant22Skill = enchant22skill;
	}
	...
	public int getEnchant22skillId()
	{
		return _enchant22Skill;
	}
	...
	/**
	 * Checks if all parts of set are enchanted to +22 or more
	 * @param player
	 * @return
	 */
	public boolean isEnchanted22(Player player)
	{
		final Inventory inv = player.getInventory();
		
		final ItemInstance chestItem = inv.getPaperdollItem(Inventory.PAPERDOLL_CHEST);
		if (chestItem.getEnchantLevel() < 22)
			return false;
		...
		final ItemInstance legsItem = inv.getPaperdollItem(Inventory.PAPERDOLL_LEGS);
		if (legsItem != null && legsItem.getEnchantLevel() > 21)
			legs = legsItem.getItemId();
		...
		final ItemInstance headItem = inv.getPaperdollItem(Inventory.PAPERDOLL_HEAD);
		if (headItem != null && headItem.getEnchantLevel() > 21)
			head = headItem.getItemId();
		...
		final ItemInstance glovesItem = inv.getPaperdollItem(Inventory.PAPERDOLL_GLOVES);
		if (glovesItem != null && glovesItem.getEnchantLevel() > 21)
			gloves = glovesItem.getItemId();
		...
		final ItemInstance feetItem = inv.getPaperdollItem(Inventory.PAPERDOLL_FEET);
		if (feetItem != null && feetItem.getEnchantLevel() > 21)
			feet = feetItem.getItemId();
