	...
	public void broadcastPacket(L2GameServerPacket packet)
	{
		...
		for (L2Character character : _characterList)
		{
			...
			*/
			...
			if (character instanceof Player && !(((Player) character).isInsideZone(ZoneId.TOURNAMENT) && !((Player) character).isOlympiadProtection()&& !((Player) character).inObserverMode()) && !((Player) character).isArenaProtection() && !((Player) character).isArenaObserv())
				character.sendPacket(packet);
		}
	}
