...
public class L2Npc extends L2Character
{
	...
	{
		// Send a packet SocialAction to all Player in the _KnownPlayers of the L2Npc
	 ...
	 * <BR>
	 * <li>Set the L2Npc as target of the Player player (if necessary)</li> <li>Send MyTargetSelected to the Player player (display the select window)</li> <li>If L2Npc is autoAttackable, send StatusUpdate to the Player in order to update L2Npc HP bar</li> <li>Send ValidateLocation
	 ...
	 * <BR>
	 * <li>Send MyTargetSelected to the Player player (display the select window)</li> <li>If L2Npc is autoAttackable, notify the Player AI with ATTACK (after a height verification)</li> <li>If L2Npc is NOT autoAttackable, notify the Player AI with INTERACT (after a distance
	 ...
	 * <BR>
	 * @param player The Player that start an action on the L2Npc
	 */
	@Override
	 ...
	 * <BR>
	 * <li>Set the L2Npc as target of the Player player (if necessary)</li> <li>Send MyTargetSelected to the Player player (display the select window)</li> <li>If L2Npc is autoAttackable, send StatusUpdate to the Player in order to update L2Npc HP bar</li> <li>Send a Server->Client
	...
	public void onActionShift(Player player)
	{
		...
		if (player.getTarget() != this)
		{
			...
			// Send MyTargetSelected to the Player player
			player.sendPacket(new MyTargetSelected(getObjectId(), player.getLevel() - getLevel()));
			
			// Check if the player is attackable (without a forced attack)
			if (isAutoAttackable(player))
			{
				// Send StatusUpdate of the L2Npc to the Player to update its HP bar
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the text of the quest state in the folder data/scripts/quests/questId/stateId.htm</li> <li>Send a Server->Client NpcHtmlMessage containing the text of the L2NpcInstance to the Player</li> <li>Send a Server->Client ActionFailed to the Player in order to avoid that the
	 ...
	 * <B><U> Actions</U> :</B><BR>
	 * <BR>
	 * <li>Get the text of the selected HTML file in function of the npcId and of the page number</li> <li>Send a Server->Client NpcHtmlMessage containing the text of the L2Npc to the Player</li> <li>Send a Server->Client ActionFailed to the Player in order to avoid that the client wait
	 ...
	 * Server->Client packet StatusUpdate with current HP and MP to all other Player to inform</li> <li>Notify L2Character AI</li><BR>
	 * <BR>
	 * <B><U> Overriden in </U> :</B><BR>
	 * <BR>
