...
{
	protected Map<Player, Long> _waterTask = new ConcurrentHashMap<>();
	
	public WaterTaskManager()
	{
		ThreadPool.scheduleAtFixedRate(new WaterScheduler(), 0, 1000);
	}
	...
	public void add(Player actor, long time)
	{
	...
	private class WaterScheduler implements Runnable
	{
		...
		public void run()
		{
			if (!_waterTask.isEmpty())
			{
				...
				synchronized (this)
				{
					for (Player actor : _waterTask.keySet())
					{
