	...
	public final void run()
	{
		...
		for (Entry<ItemInstance, Player> entry : _shadowItems.entrySet())
		{
			...
			if (mana == -1)
			{
				...
				player.sendMessage(""+item.getItemName()+" Time is over.");
				_shadowItems.remove(item);
			...
			if (mana == 60 - DELAY)
				player.sendMessage(""+item.getItemName()+" Remaining minute is now 1.");
			else if (mana == 300 - DELAY)
				player.sendMessage(""+item.getItemName()+" Remaining minute(s) is now 5.");
			else if (mana == 600 - DELAY)
				player.sendMessage(""+item.getItemName()+" Remaining minute(s) is now 10.");
