...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if ((command.startsWith("admin_character_list")) || (command.startsWith("admin_character_info")))
		{
			try
			{
				...
				Player target = L2World.getInstance().getPlayer(val);
				if (target != null)
		...
		else if (command.startsWith("admin_party_info"))
		{
			...
			if (target instanceof Player)
			{
				if (((Player) target).isInParty())
					...
					activeChar.sendMessage(((Player) target).getName() + " isn't in a party.");
			}
			else
				activeChar.sendPacket(SystemMessageId.INCORRECT_TARGET);
		}
	...
	{
		final Collection<Player> allPlayers = L2World.getInstance().getAllPlayers().values();
	...
	private static void showCharacterInfo(Player activeChar, Player player)
	{
		if (player == null)
		{
			L2Object target = activeChar.getTarget();
	 ...
	 * @throws IllegalArgumentException
	 */
	private static void findCharactersPerAccount(Player activeChar, String characterName) throws IllegalArgumentException
	{
		if (Util.isValidPlayerName(characterName))
		{
			...
			Player player = L2World.getInstance().getPlayer(characterName);
			if (player == null)
