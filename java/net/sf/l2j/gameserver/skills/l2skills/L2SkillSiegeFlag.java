...
public class L2SkillSiegeFlag extends L2Skill
{
	...
	public void useSkill(L2Character activeChar, L2Object[] targets)
	{
		...
		NpcTemplate flagTemplate = NpcTable.getInstance().getTemplate(35062);
		final L2SiegeFlagInstance flag = new L2SiegeFlagInstance(player, IdFactory.getInstance().getNextId(), flagTemplate);
		
		flag.setTitle(player.getClan().getName());
		flag.setCurrentHp(flag.getMaxHp());
		flag.setHeading(player.getHeading());
		flag.spawnMe(player.getX(), player.getY(), player.getZ() + 50);
		flag.setIsRunning(false); // broadcast info
		flagSpawned = true;
	}
