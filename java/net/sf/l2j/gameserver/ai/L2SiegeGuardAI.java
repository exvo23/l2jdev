		...
		if (willCastASpell())
		{
			...
			if (!defaultList.isEmpty())
			{
				...
				for (L2Character cha : actor.getKnownList().getKnownTypeInRadius(L2Character.class, 1000))
				{
					...
					if (!actor.isAttackingDisabled() && (cha instanceof Player && actor.getCastle().getSiege().checkIsDefender(((Player) cha).getClan())) || (cha instanceof L2Npc && Util.contains(clans, ((L2Npc) cha).getClans())))
					{
